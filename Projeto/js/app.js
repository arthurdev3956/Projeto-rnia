...
const getTasks = async () => { //função para buscar a lista de tarefa na api
  const apiResponse = await fetch("https://json-server-production-185b.up.railway.app/Posts");
  const resultTask = await apiResponse.json();
  return resultTask;
};
const getTask = async (id) => { //função para buscar uma tarefa específica na ap
  const apiResponse = await fetch(`https://json-server-production-185b.up.railway.app/Posts/${id}`);
...
const createTask = async (tasks) => { //função que cria a tarefa
  await fetch("https://json-server-production-185b.up.railway.app/Posts", {
    method: "POST",
    headers: {
...
const updateTask = async (id, tasks) => { //essa função é para editar a tarefa
  await fetch(`https://json-server-production-185b.up.railway.app/Posts/${id}`, {
    method: "PUT",
    headers: {
      Accept: "application/json, text/plain, */*",
      "Content-Type": "application/json",
